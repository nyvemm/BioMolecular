...
module.exports = (app, database) => {
  ...
  app.get('/resultados/:id', (req, res) => {
    database('resultados')
      .select()
      .where('amostra_contem_exames_aux.idAmostra', req.params.id)
      .innerJoin(
  ...
  app.put('/amostra-resultados', (req, res) => {
    ...
    database('amostra')
      .where('idAmostra', id)
      .update({
        interpretacao_resultados: req.body.interpretacao_resultados
          ? req.body.interpretacao_resultados
          : null,
              ...
              .then((idAmostra) => {
                ...
                database('amostra_contem_exames_aux')
                  ...
                  .then((amostras) => {
                    ...
                    try {
                      const finalizadas = amostras.filter(
                        (amEx) => amEx.status
                      );
                      let FinalStatus = '';
  ...
  app.get('/amostras/:id', loggedIn, (req, res) => {
    DAOAmostra.getAmostra(req.params.id)
      .then((data) => {
        try {
          const amostraAtual = data[0];
          amostraAtual.cadastrado_em = viewDateFormat(
            amostraAtual.cadastrado_em
          );
          amostraAtual.f_dt_recebimento = viewDateFormat(
            amostraAtual.dt_recebimento
          );
          amostraAtual.f_dt_solicitacao = viewDateFormat(
            amostraAtual.dt_solicitacao
          );
          amostraAtual.f_dt_coleta = viewDateFormat(amostraAtual.dt_coleta);
          amostraAtual.f_dt_ult_transfusao = viewDateFormat(
            amostraAtual.dt_ult_transfusao
          );
          ...
          if (amostraAtual.dt_coleta) {
            const diffTime = Math.abs(
              amostraAtual.dt_coleta - amostraAtual.dt_nasc
            );
          ...
          database('amostra_contem_exames_aux')
            ...
            .then((exame) => {
              ...
              res.render('amostras/visualizar', {
                data: amostraAtual,
                exame: exameAtual
              });
            })
  ...
  app.get('/editar-amostra/:id', loggedIn, (req, res) => {
    DAOAmostra.getAmostra(req.params.id).then((data) => {
      try {
        const amostraAtual = data[0];
        amostraAtual.cadastrado_em = inputDateFormat(
          amostraAtual.cadastrado_em
        );
        amostraAtual.dt_recebimento = inputDateFormat(
          amostraAtual.dt_recebimento
        );
        amostraAtual.dt_solicitacao = inputDateFormat(
          amostraAtual.dt_solicitacao
        );
        amostraAtual.dt_coleta = inputDateFormat(amostraAtual.dt_coleta);
        amostraAtual.dt_ult_transfusao = inputDateFormat(
          amostraAtual.dt_ult_transfusao
        );
        amostraAtual.medicamentos =
          amostraAtual.medicamentos &&
          JSON.parse(amostraAtual.medicamentos).length > 0
            ? JSON.parse(amostraAtual.medicamentos).split(',')
            : null;
  ...
  app.get('/cadastrar-resultado/:id', loggedIn, (req, res) => {
    database('amostra_contem_exames_aux')
      .select(
        database.raw(
          '*, exame.nome as exame_nome, paciente.nome as paciente_nome, amostra.observacao as observacao'
        )
      )
          ...
          .then((tiposExames) => {
            try {
              ...
              exames.forEach((exame) => {
                ...
                exameAtual.tem_possiveis_valores =
                  exameAtual.tipo_resultado === 'Possíveis Valores';
                exameAtual.valores = JSON.parse(
                  exameAtual.possiveis_resultados
                );
                if (exameAtual.tipo_resultado === 'Texto') {
                  exameAtual.tipoInput = 'text';
                } else if (exameAtual.tipo_resultado === 'Numérico') {
                  exameAtual.tipoInput = 'number';
                } else {
                  exameAtual.tipoInput = 'text';
                }
                const intervaloValor =
                  ...
                  exameAtual.tabela_intervalo !== ''
                    ? JSON.parse(exameAtual.tabela_intervalo)
                    : null;

                const diffTime = Math.abs(
                  exameAtual.dt_coleta - exameAtual.dt_nasc
                );
                const diffDays = Math.floor(diffTime / (1000 * 60 * 60 * 24));
                exameAtual.idade_coleta = Math.floor(diffDays / 365);
                exameAtual.idade_coleta_meses = Math.floor(
                  (diffDays % 365) / 30
                );
                exameAtual.intervalo_bool =
                  exameAtual.tipo_valor_ref === 'Idade';
                ...
                if (intervaloValor) {
                  intervaloValor.forEach((it) => {
                    if (it.idade.startsWith('>')) {
                      if (
                        exameAtual.idade_coleta >
                        parseInt(it.idade.substring(1), 10)
                      ) {
                      ...
                      if (
                        parseInt(
                          it.idade.substring(0, it.idade.indexOf('-')) <=
                            exameAtual.idade_coleta &&
                            exameAtual.idade_coleta <=
                              parseInt(
                                it.idade.substring(it.idade.indexOf('-') + 1),
                                10
                              ),
                          10
                        )
                      ...
                      parseInt(it.idade, 10) === exameAtual.idade_coleta
                    ) {
                      exameAtual.intervalo_certo = it.valor;
                    }
                  });
