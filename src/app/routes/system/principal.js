...
import { loggedIn, visitanteLogin } from '../../helpers/login.js';
...
async function getData() {
  try {
    const nPacientes = await database('paciente').select().count({ count: '*' });
    const nExames = await database('exame').select().count({ count: '*' });
    const nSolicitantes = await database('solicitante').select().count({ count: '*' });
    const nAmostras = await database('amostra').select().count({ count: '*' });
    const nAmostrasP = await database('amostra').select().where('status_pedido', 'Parcialmente avaliado').count({ count: '*' });
    const nAmostrasNA = await database('amostra').select().where('status_pedido', 'Não avaliado').count({ count: '*' });
...
async function getProximasAmostras() {
  try {
    const amostras = await database('amostra').whereIn('status_pedido', ['Não avaliado', 'Parcialmente avaliado'])
    ...
    for (let i = 0; i < amostras.length; i += 1) {
      amostras[i].em_analise = amostras[i].status_pedido === 'Parcialmente avaliado';
...
export default (app) => {
  ...
  app.get('/visitante', visitanteLogin, (req, res) => {
    ...
    database('solicitante').where('login_visitante', req.user.login).select('idSolicitante').first()
      .then((solicitante) => {
        // Pega as amostras pelo id do solicitante.
        database('amostra').where('amostra.idSolicitante', solicitante.idSolicitante)
          ...
          .select(database.raw('*, paciente.nome as paciente_nome, solicitante.nome as solicitante_nome, amostra.observacao as observacao'))
          .then((amostrasData) => {
            ...
            for (let i = 0; i < amostras.length; i += 1) {
              ...
              amostras[i].f_dt_coleta = viewDateFormat(amostras[i].dt_coleta);
              amostras[i].cadastrado_em = viewDateFormat(amostras[i].cadastrado_em);
              amostras[i].f_dt_recebimento = viewDateFormat(amostras[i].dt_recebimento);
              amostras[i].f_dt_solicitacao = viewDateFormat(amostras[i].dt_solicitacao);
              amostras[i].f_dt_coleta = viewDateFormat(amostras[i].dt_coleta);
              amostras[i].f_dt_ult_transfusao = viewDateFormat(amostras[i].dt_ult_transfusao);
              amostras[i].finalizado = amostras[i].status_pedido === 'Finalizado';

              /* Idade do Paciente */
              if (amostras[i].dt_coleta) {
                const diffTimeColeta = Math.abs(amostras[i].dt_coleta - amostras[i].dt_nasc);
                const diffDaysColeta = Math.floor(diffTimeColeta / (1000 * 60 * 60 * 24));
                amostras[i].idade_coleta = Math.floor(diffDaysColeta / 365);
                amostras[i].idade_coleta_meses = Math.floor((diffDaysColeta % 365) / 30);
              }

              database('amostra_contem_exames_aux').where('idAmostra', amostras[i].idAmostra)
                ...
                .then((exame) => {
                  ...
                  res.render('solicitantes/visitante', { amostras, qtdAmostras: amostras.length });
                })
                .catch((error) => {
